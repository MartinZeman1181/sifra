...
public class SHA2NBLOCK {
    ...
    public long[][] generateHash(byte[] zdroj, int[] prad) throws UnsupportedEncodingException {
        byte[] b = new byte[128];
        byte[] bb = BigInteger.valueOf(zdroj.length << 3).toByteArray();
        int delka2 = bb.length + 1;
        int pocetBloku = zdroj.length / 128;
        int radekMatice = 0;
        long[][] M = new long[pocetBloku + 2][16];
        for (int i = 0; i < pocetBloku; i++) {
            int offset = i << 7;
            radekMatice = i / 128;
            createBlock(M, zdroj, offset, radekMatice);
        }
        int zbyva = zdroj.length - 128 * pocetBloku;
        if (zbyva == 0) {
            b[0] = (byte) 0x80;
            for (int i = 1; i < delka2; i++) {
                b[i] = 0x00;
            }
            int ii = 0;
            for (int i = bb.length; i < 128 - delka2 + 1; i++) {
                b[i] = 0x00;
            }
            ii = 0;
            for (int i = 128 - delka2 + 1; i < 128; i++) {
                b[i] = bb[ii++];
            }
            if (pocetBloku > 0) {
                radekMatice = radekMatice + 1;
            } else {
                System.out.println("Prázdný vstup");
            }
            createBlock(M, b, 0, radekMatice);            
        } else {
            int k = 896 - (zbyva << 3) - 1;
            if (k > 0) {
                for (int i = 0; i < zbyva; i++) {
                    b[i] = zdroj[(pocetBloku << 7) + i];
                }
                b[zbyva] = (byte) 0x80;
                for (int i = zbyva + 1; i < 128 - bb.length; i++) {
                    b[i] = 0x00;
                }
                int i1 = 128 - bb.length;
                int ii = 0;
                for (int i = i1; i < 128; i++) {
                    b[i] = bb[ii++];
                }
                if (pocetBloku > 0) {
                    radekMatice = radekMatice + 1;
    ...
    private void createBlock(long[][] M, byte[] zdroj, int offset, int radekMatice) throws UnsupportedEncodingException {
        /* bloky 128 bytes = offset je třeba posouvat po bytovém řetězci s periodou 128 bytů */
        //long[] blk = new long[16];
        long pom = 0;
        for (int k = 0; k < 16; k++) {
            M[radekMatice][k] = 0;
            int kk = (k << 3) + offset;
            for (int j = 0; j < 8; j++) {
                ...
                M[radekMatice][k] = (M[radekMatice][k] & 0xffffffffffffffffL) + (pom & 0xffffffffffffffffL);
                /*
                String tiskSlova = tiskSlova(M[radekMatice][k]);
                System.out.println(tiskSlova);    //Zde vzniká chyba, která změní třetí byt s hodnotou 0x63 na hodnotu 0x62
                 */
            }
    ...
    public String hash(long[][] bloky, int pocetBloku) throws UnsupportedEncodingException {
        ...
        for (int iblok = 0; iblok < pocetBloku; iblok++) {

            for (int i = 0; i < 16; i++) {
                w[i] = bloky[iblok][i];
                //String tiskSlova = tiskSlova(w[i]);
                //System.out.println(tiskSlova);
            }
        ...
        for i from 16 to 63
            ...
            long a = hx[0] & 0xffffffffffffffffL;
            long b = hx[1] & 0xffffffffffffffffL;
            long c = hx[2] & 0xffffffffffffffffL;
            long d = hx[3] & 0xffffffffffffffffL;
            long e = hx[4] & 0xffffffffffffffffL;
            long f = hx[5] & 0xffffffffffffffffL;
            long g = hx[6] & 0xffffffffffffffffL;
            long h = hx[7] & 0xffffffffffffffffL;
    ...
    public String vytvorHash(String text) throws UnsupportedEncodingException {
     byte[] textArea = text.getBytes("UTF-8");
        int pocetRadku = 0;
        int[] prad = new int[1];
        long[][] matice = generateHash(textArea, prad);
        pocetRadku = prad[0];
        String hashx = hash(matice, pocetRadku);
        return hashx; 
    
    }

    public static void main(String[] arg) throws UnsupportedEncodingException {
        SHA2NBLOCK sha2 = new SHA2NBLOCK();
        //sha2.hash();
        /*
        ...
        int iii;
         */
        //String text = "01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567";
        //String text = "abc";
        //String text = "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrl";
        //        + "mnopqrsmnopqrstnopqrstu";
        //String text = "Vytřel bych ti zrak, co dokáže můj pták - leh bych já si v znak, vyzved' bych tě do oblak";
        String text = "борщ";
        //String text = StringUtils.repeat("a", 100000);
        byte[] textArea = text.getBytes("UTF-8");
        int pocetRadku = 0;
        int[] prad = new int[1];
        long[][] matice = sha2.generateHash(textArea, prad);
        pocetRadku = prad[0];
        /*
        for (int i = 0; i < pocetRadku; i++) {
            System.out.format("radek = %d\n", i + 1);
            for (int j = 0; j < 16; j++) {
                String tiskSlova = sha2.tiskSlova(matice[i][j]);
                System.out.format("%s\n", tiskSlova);
            }
        };
        */
        String hashx = sha2.hash(matice, pocetRadku);
